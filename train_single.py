...
from arguments import ModelParams, PipelineParams, OptimizationParams, SubdivisionParams
from train import update_params_with_config
from scene.combine import *
from scene.subdivision import Box
...
def training_parallel_single(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, subdiv, subdivW, subdivH, iterationS, unique_str_all, block,  subd_params):

    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset, iterationS, unique_str_all)
    ...
    bg_color = [1, 1, 1] if dataset.white_background else [0, 1, 0]
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        #### Use the probablilities - more details
        ...
        if opt.preload:
            ...
            uncertainthreshhold = subd_params.uncertainthreshhold #0.15 for real attic scene, 0.2 for testdach+kitchen+museum, 0.3 for meeting room+Caterpillar
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                if not subdiv:
                    ...
                    scene.save(iteration, subdiv, block)
                continue
...
def prepare_output_and_logger(args, iterationS, unique_str_all):    
    if not args.model_path:
        if os.getenv('OAR_JOB_ID'):
            unique_str=os.getenv('OAR_JOB_ID')
        else:
            unique_str = unique_str_all
        args.model_path = os.path.join("./output/", unique_str[0:10], str(iterationS))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[7_000, 15_000, 25_000])
    ...
    parser.add_argument("--unique_str_all", type=str, default = None)
    parser.add_argument("--block", nargs="+", type=float, default=[1.0,2.0,3.0,10.0,20.0,5.0])
    ...
    ### init block
    block = Box(args.block[0], args.block[1], args.block[2], args.block[3], args.block[4], args.block[5], 0)
    training_parallel_single(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.subdiv, args.subdivW, args.subdivH, args.iterationS, args.unique_str_all, block, subd.extract(args))
